mod simple_garbage_collector {
    ...
    pub fn merge_free_oops(space: &mut MemorySpace) {
        let mut iter = space.iter();
        while let Some(mut current_oop) = iter.next() {
			if let Some(mut next_oop) = iter.next() {
				if current_oop.is_free_oop() && next_oop.is_free_oop() { 
					// Merged oops only need one header !
					let new_number_of_slots = current_oop.get_header().number_of_slots_bits() + next_oop.get_header().oop_size();
					current_oop
						.get_header()
